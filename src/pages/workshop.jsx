...
export default function WorkshopBrandLayout() {
  const [selected, setSelected] = useState(null);
  const [works, setWorks] = useState([]);
  const [content, setContent] = useState({ title: "Workshop", desc: "" });
  const navigate = useNavigate();

  useEffect(() => {
    const fetchData = async () => {
      // Filter khusus untuk kategori 'workshop'
      ...
      const { data: contentData } = await supabase.from("content").select("*").eq("section", "workshop");
      if (contentData) {
        setContent({
          title: contentData.find(c => c.key === "title")?.value || "Workshop",
