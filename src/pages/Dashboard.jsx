...
import { 
  ...
  Activity,
  Trash2,
  Menu, // Tambahan ikon untuk menu HP
  X // Tambahan ikon untuk tutup menu HP
} from "lucide-react";
...
export default function Dashboard() {
  ...
  // State untuk membuka/tutup sidebar di HP
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  // ─── KONDISI ROLE MANAGEMENT ──────────────────────────────────────────────
  ...
  // Tutup sidebar otomatis setiap kali berpindah halaman di HP
  useEffect(() => {
  ...
  const handleDeleteActivity = async (id, targetName) => {
    if (!isSuperAdmin) {
      alert("Akses ditolak! Hanya Akun Superadmin yang dapat menghapus log.");
      return;
    }
  ...
  return (
    <div className="min-h-screen flex bg-[#0a0a0a] text-zinc-200 font-sans selection:bg-indigo-500/30 relative overflow-x-hidden">
      ...
      {/* OVERLAY BACKGROUND (Muncul hanya saat sidebar terbuka di HP) */}
      {isSidebarOpen && (
        <div 
      ...
      {/* SIDEBAR (Responsive: Hidden di HP, sliding drawer jika di-toggle) */}
      <aside className={`
      ...
      `}>
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-base font-semibold tracking-wide text-white">Workspace</span>
          </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          <p className="px-3 text-xs font-semibold text-zinc-500 mb-2 uppercase tracking-wider">Overview</p>
          ...
          <p className="px-3 text-xs font-semibold text-zinc-500 mb-2 uppercase tracking-wider">System</p>
          ...
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="flex-1 overflow-hidden">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {isSuperAdmin && <span className="text-[9px] bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded border border-red-500/30 font-bold uppercase">Superadmin</span>}
                {isAdminBiasa && <span className="text-[9px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded border border-white/5 font-bold uppercase">Admin</span>}
              </div>
              <p className="text-xs text-zinc-500 truncate">{adminEmail || "Loading profile..."}</p>
            </div>
          </div>
          <button onClick={logout} className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg text-sm font-medium text-zinc-400 hover:text-red-400 hover:bg-red-400/10 transition-all">
      ...
      <div className="flex-1 flex flex-col min-w-0 w-full">
        ...
        {/* TOP HEADER (Responsive Search & Burger Menu) */}
          ...
          {location.pathname === "/admin/dashboard" && (
            <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
              ...
              <div className="mb-6 md:mb-8">
                ...
                <p className="text-xs md:text-sm text-zinc-500 mt-1">Monitor your website metrics and recent updates in real-time.</p>
              </div>

              {/* STAT CARDS (Responsive: 1 kolom di HP, 3 kolom di Desktop) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5 mb-6 md:mb-8">
                <div className="bg-[#111111] border border-white/5 rounded-xl p-5 shadow-sm">
                  <div className="p-2 bg-indigo-500/10 rounded-lg text-indigo-400 w-fit mb-4"><FolderDot className="w-5 h-5" /></div>
                  <h3 className="text-2xl md:text-3xl font-semibold text-white">12</h3>
                  <p className="text-xs md:text-sm text-zinc-500 mt-1">Total Active Projects</p>
                </div>

                <div className="bg-[#111111] border border-white/5 rounded-xl p-5 shadow-sm">
                  <div className="p-2 bg-sky-500/10 rounded-lg text-sky-400 w-fit mb-4"><HardDriveUpload className="w-5 h-5" /></div>
                  <h3 className="text-2xl md:text-3xl font-semibold text-white">84</h3>
                  <p className="text-xs md:text-sm text-zinc-500 mt-1">Media Files Uploaded</p>
                </div>

                <div className="bg-[#111111] border border-white/5 rounded-xl p-5 shadow-sm sm:col-span-2 md:col-span-1">
                  <div className="p-2 bg-amber-500/10 rounded-lg text-amber-400 w-fit mb-4"><Users className="w-5 h-5" /></div>
                  <h3 className="text-2xl md:text-3xl font-semibold text-white">{loadingStats ? "..." : totalAdmins}</h3>
                  <p className="text-xs md:text-sm text-zinc-500 mt-1">Registered Admins</p>
                </div>
              </div>

              {/* LOG TABLE (Responsive: Dilengkapi horizontal scroll wrapper) */}
              <div className="bg-[#111111] border border-white/5 rounded-xl overflow-hidden shadow-sm">
                <div className="px-4 md:px-6 py-4 border-b border-white/5 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm md:text-base font-semibold text-white">Recent Activity Log</h2>
                  </div>
                  <span className="text-[10px] md:text-xs text-zinc-500 bg-white/5 px-2 py-1 rounded-md">Live</span>
                </div>
                
                {/* Pembungkus Table scrollable untuk HP */}
                <div className="overflow-x-auto w-full current-scrollbar">
                  <table className="w-full text-xs md:text-sm text-left min-w-[600px] md:min-w-full">
                    <thead className="text-[11px] md:text-xs text-zinc-500 bg-white/[0.02] border-b border-white/5">
                      <tr>
                        <th className="px-4 md:px-6 py-3 font-medium">Target Context</th>
                        <th className="px-6 py-3 font-medium">Action</th>
                        <th className="px-6 py-3 font-medium">Time / Date</th>
                        <th className="px-6 py-3 font-medium">Executor</th>
                        {isSuperAdmin && <th className="px-4 md:px-6 py-3 font-medium text-right">Options</th>}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {loadingActivities ? (
                        <tr>
                          <td colSpan={isSuperAdmin ? "5" : "4"} className="px-6 py-10 text-center text-zinc-500">Loading logs...</td>
                        </tr>
                      ) : activities.length > 0 ? (
                        activities.map((activity) => (
                          <tr key={activity.id} className="hover:bg-white/[0.02] transition-colors">
                            <td className="px-4 md:px-6 py-4 font-medium text-white flex items-center gap-3">
                              ...
                              <span className="truncate max-w-[150px] md:max-w-none">{activity.target_name}</span>
                            </td>
                        ...
                        <tr>
                          <td colSpan={isSuperAdmin ? "5" : "4"} className="px-6 py-10 text-center text-zinc-500">No activities recorded yet.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
