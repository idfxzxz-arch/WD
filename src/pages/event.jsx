...
export default function EventBrandLayout() {
  const [selected, setSelected] = useState(null);
  const [works, setWorks] = useState([]);
  const [content, setContent] = useState({ title: "Event Production", desc: "" });
  const navigate = useNavigate();

  useEffect(() => {
    const fetchData = async () => {
      // Mengambil data dari kategori 'production' dan 'workshop'
      const { data: worksData } = await supabase
        .from("works")
        .select("*")
        .in("category", ["production", "workshop"])
        .order("order_index");
      
      if (worksData) setWorks(worksData);

      // Mengambil konten teks untuk section event
      const { data: contentData } = await supabase.from("content").select("*").eq("section", "event");
      if (contentData) {
        setContent({
          title: contentData.find(c => c.key === "title")?.value || "Event Production & Workshop",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {works.map((item) => (
            <motion.div key={item.id} whileInView={{ scale: 1 }} initial={{ scale: 0.95 }} className="bg-white p-8 rounded-2xl shadow-sm border cursor-pointer" onClick={() => setSelected(item.image)}>
              <div className="text-[10px] text-neutral-400 mb-4 uppercase font-mono">{item.category}</div>
